...
publish:
	...
	@if [ $(NUGET_VALID) -eq 0 ]; then \
		nuget push Pdfcrowd.Official.4.3.5.nupkg -Source https://www.nuget.org/api/v2/package -ApiKey $(API_KEY) ; \
	else \
		echo ; \
		echo Not valid nuget version, upload Pdfcrowd.Official.4.3.5.nupkg manually to https://www.nuget.org/packages/manage/upload ; \
		echo ; \
	fi
